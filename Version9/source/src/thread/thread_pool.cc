...
void ThreadPool::parallelFor(size_t width, size_t height, const std::function<void(size_t, size_t)> &lambda, bool complex){ // 并行for循环
    ...
    float chunk_width_float = static_cast<float>(width) / sqrt(threads.size());  // 每个线程处理的像素块大小
    float chunk_height_float = static_cast<float>(height) / sqrt(threads.size()); // 每个线程处理的像素块大小
    if (complex){
        chunk_width_float /= sqrt(16);  // 每个线程处理的像素块大小
        chunk_height_float /= sqrt(16); // 每个线程处理的像素块大小
    }
